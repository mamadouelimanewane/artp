...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`signum-sidebar w-60 shrink-0 min-h-screen flex flex-col text-white${open ? " open" : ""}`}
      ...
      >
        ...
        <div className="px-5 py-6 border-b border-white/10">
          <div className="flex items-center justify-between mb-3">
            ...
            {/* Bouton fermer — visible uniquement mobile */}
            <button
              onClick={onClose}
        ...
        <nav className="flex-1 py-4 px-3 overflow-auto">
          ...
          <div className="pt-2 border-t border-white/10 mb-3 space-y-0.5">
            {NAV_CORE.map(n => (
              <NavLink key={n.to} to={n.to} end={n.to === "/dashboard"} onClick={onClose}
                className={({ isActive }) => linkClass(isActive)}
                style={({ isActive }) => isActive
